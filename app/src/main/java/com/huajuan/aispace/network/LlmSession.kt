...
class LlmSession(
    ...
    fun load(context: Context) {
        ...
        debugLog(TAG) { "开始加载模型: $actualModelDir, 实际路径: $actualModelPath" }
        modelLoading = true
        ...
        debugLog(TAG) { "模型加载完成，nativePtr: $nativePtr" }
    }
    ...
    private fun extractModelFromAssets(context: Context, assetsPath: String): String {
        try {
            ...
            if (!modelDir.exists()) {
                debugLog(TAG) { "创建模型目录: ${modelDir.absolutePath}" }
                modelDir.mkdirs()
            }
            ...
            debugLog(TAG) { "模型文件提取完成: ${modelFile.absolutePath}, 大小: ${modelFile.length()} bytes" }
    ...
    private fun copyAssetDir(context: Context, assetDir: String, targetDir: File) {
        try {
            debugLog(TAG) { "开始复制assets目录: $assetDir -> ${targetDir.absolutePath}" }
            val assets = context.assets.list(assetDir) ?: run {
                Log.w(TAG, "assets目录为空或不存在: $assetDir")
                return
            }
            
            debugLog(TAG) { "assets目录内容: ${assets.joinToString(", ")}" }
            ...
            for (fileName in assets) {
                ...
                if (isDirectory) {
                    ...
                    if (!subDir.exists()) {
                        debugLog(TAG) { "创建子目录: ${subDir.absolutePath}" }
                        subDir.mkdirs()
                    }
                    ...
                    if (!destFile.exists() || destFile.length() == 0L) {
                        debugLog(TAG) { "正在复制模型文件: $assetPath -> ${destFile.absolutePath}" }
                        context.assets.open(assetPath).use { inputStream ->
                            FileOutputStream(destFile).use { outputStream ->
                                ...
                                outputStream.flush()
                                debugLog(TAG) {
                                    "文件复制完成: ${destFile.absolutePath}, 大小: ${destFile.length()} bytes, copied: $totalBytes bytes"
                                }
                                ...
                                debugLog(TAG) { "验证文件大小: ${destFile.absolutePath}, size: $verifiedSize bytes" }
                                
                                // 如果是.mnn或权重文件，特别验证
                                if (fileName.endsWith(".mnn") || fileName.contains(".mnn.weight")) {
                                    debugLog(TAG) { "复制了关键模型文件: $fileName, size: $verifiedSize bytes" }
                                }
                            }
                        }
                    } else {
                        debugLog(TAG) { "模型文件已存在，跳过: ${destFile.absolutePath}, 大小: ${destFile.length()} bytes" }
                        ...
                        if (fileName.endsWith(".mnn") || fileName.contains(".mnn.weight")) {
                            ...
                            if (destFile.length() != originalSize) {
                                ...
                                debugLog(TAG) { "删除不完整文件，将重新复制: ${destFile.absolutePath}" }
                                ...
                                context.assets.open(assetPath).use { inputStream ->
                                    FileOutputStream(destFile).use { outputStream ->
                                        ...
                                        outputStream.flush()
                                        debugLog(TAG) {
                                            "重新复制完成: ${destFile.absolutePath}, 大小: ${destFile.length()} bytes, copied: $totalBytes bytes"
                                        }
            ...
            debugLog(TAG) { "完成复制assets目录: $assetDir" }
        } catch (e: Exception) {
    ...
    fun release() {
        ...
        synchronized(this) {
            debugLog(TAG) { "释放LLM会话资源: nativePtr=$nativePtr, generating=$generating" }
    ...
    private fun ensureNativeReady() {
        if (!isNativeAvailable) {
            val reason = nativeLoadError ?: "unknown error"
            throw IllegalStateException("本地推理库 mnnllmapp 不可用: $reason")
        }
    }
    ...
    companion object {
        ...
        init {
            try {
                ...
                Log.e(TAG, "加载本地推理库失败，当前将禁用本地模型能力", e)
            } catch (e: SecurityException) {
                isNativeAvailable = false
                nativeLoadError = e.message
                Log.e(TAG, "加载本地推理库被系统拒绝，当前将禁用本地模型能力", e)
            }
        }
    }
}
